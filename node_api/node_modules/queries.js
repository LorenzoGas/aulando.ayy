...
var auleLibere = "SELECT a.nome, MIN(DATE_FORMAT(inizio, '%H:%i'))as minimo FROM Aula a JOIN Dipartimento d ON a.dipartimento = d.id AND d.id = ? LEFT JOIN AulaLezione al ON al.aula = a.id LEFT JOIN Lezione l ON al.lezione = l.id AND l.giorno = ? AND l.inizio > ? WHERE a.id NOT IN( SELECT a.id FROM	Aula a JOIN AulaLezione al ON a.id = al.aula JOIN Lezione l ON l.id = al.lezione WHERE	l.giorno = ? AND l.inizio <= ? AND l.fine > ?) GROUP BY a.nome ORDER BY IFNULL(MIN(l.inizio),'24:00') DESC";
...
var orariAula = "SELECT d.cognomenome, m.nome, DATE_FORMAT(inizio, '%H:%i'), DATE_FORMAT(fine, '%H:%i') FROM Aula a JOIN Dipartimento di ON a.dipartimento = di.id AND di.id = ? JOIN AulaLezione al ON a.id = al.aula JOIN Lezione l ON al.lezione = l.id JOIN Materia m ON l.materia = m.id JOIN Docente d ON l.docente = d.id WHERE a.nome = ? AND l.giorno = ? ORDER BY l.inizio";
