...
var orariAula = "SELECT d.cognomenome, m.nome, DATE_FORMAT(inizio, '%H:%i') as inizio, DATE_FORMAT(fine, '%H:%i') as fine FROM Aula a JOIN Dipartimento di ON a.dipartimento = di.id AND di.id = ? JOIN AulaLezione al ON a.id = al.aula JOIN Lezione l ON al.lezione = l.id JOIN Materia m ON l.materia = m.id JOIN Docente d ON l.docente = d.id WHERE a.nome = ? AND l.giorno = ? ORDER BY l.inizio";
